---
Title: Welcome to Massive help document
Description: Sample Massive help document is a stupidly simple, blazing fast, flat file CMS.
---
...
ComponentSample is a flat file CMS. This means there is no administration backend or
database to deal with. You simply create `.md` files in the `content` folder
and those files become your pages. For example, this file is called `index.md`
and is shown as the main landing page.

When you install ComponentSample, it comes with some sample contents that will display
until you add your own content. Simply add some `.md` files to your `content`
folder in ComponentSample's root directory. No configuration is required, ComponentSample will
automatically use the `content` folder as soon as you create your own
`index.md`. Just check out [ComponentSample's sample contents][SampleContents] for an
example!
...
ComponentSample strictly separates contents of your website (the Markdown files in your
...
ComponentSample's default theme) use some special "hidden" file to manage meta data (like
`_meta.md` in ComponentSample's sample contents). Some other themes use a `_footer.md` to
...
These pages can't be accessed from a web browser, ComponentSample will show a 404 error
page instead.
...
files, use ComponentSample's `assets` folder. You can then access them in your Markdown
...
below). Meta headers sometimes have a special meaning: For instance, ComponentSample not
...
`Template` meta header: It controls what Twig template ComponentSample uses to display
this page (e.g. if you add `Template: blog`, ComponentSample uses `blog.twig`).
...
* <code>&#37;site_title&#37;</code> - The title of your ComponentSample site
* <code>&#37;base_url&#37;</code> - The URL to your ComponentSample site; internal links
  can be specified using <code>&#37;base_url&#37;?sub/page</code>
* <code>&#37;theme_url&#37;</code> - The URL to the currently used theme
* <code>&#37;assets_url&#37;</code> - The URL to ComponentSample's `assets` directory
* <code>&#37;themes_url&#37;</code> - The URL to ComponentSample's `themes` directory;
  don't confuse this with <code>&#37;theme_url&#37;</code>
* <code>&#37;plugins_url&#37;</code> - The URL to ComponentSample's `plugins` directory
* <code>&#37;version&#37;</code> - ComponentSample's current version string (e.g. `2.0.0`)
...
ComponentSample is not blogging software - but makes it very easy for you to use it as a
...
If you want to use ComponentSample as a blogging software, you probably want to do
...
ComponentSample is highly customizable in two different ways: On the one hand you can
change ComponentSample's appearance by using themes, on the other hand you can add new
functionality by using plugins. Doing the former includes changing ComponentSample's HTML,
...
You can create themes for your ComponentSample installation in the `themes` folder. ComponentSample
...
[ComponentSample's default theme][PicoTheme] isn't really intended to be used for a
...
the theme, and a `ComponentSample-theme.yml` to set the necessary config parameters. Just
refer to ComponentSample's default theme as an example. You can use different templates
for different content files by specifying the `Template` meta header. Simply
add e.g. `Template: blog` to the YAML header of a content file and ComponentSample will
...
* `{{ base_url }}` - The URL to your ComponentSample site; use Twig's `link` filter to
...
* `{{ assets_url }}` - The URL to ComponentSample's `assets` directory
* `{{ themes_url }}` - The URL to ComponentSample's `themes` directory; don't confuse this
                       with `{{ theme_url }}`
* `{{ plugins_url }}` - The URL to ComponentSample's `plugins` directory
* `{{ version }}` - ComponentSample's current version string (e.g. `%version%`)
...
are ComponentSample's `content` variable (e.g. `{{ content }}`), ComponentSample's `content` filter
(e.g. `{{ "sub/page"|content }}`), and ComponentSample's `markdown` filter, they all are
marked as HTML safe.

#### Dealing with pages

There are several ways to access ComponentSample's pages list. You can access the current
page's data using the `current_page` variable, or use the `prev_page` and/or
`next_page` variables to access the respective previous/next page in ComponentSample's
...
* `{{ tree_node }}` - The page's node in ComponentSample's page tree; check out ComponentSample's
                      [page tree documentation][FeaturesPageTree] for details

ComponentSample's `pages()` function is the best way to access all of your site's pages.
It uses ComponentSample's page tree to easily traverse a subset of ComponentSample's pages list. It
...
example, if you pass `pages(depthOffset=-1)`, the list will also include ComponentSample's
main index page (i.e. `content/index.md`). This one is commonly used to create
a theme's main navigation. If you want to learn more, head over to ComponentSample's
complete [`pages()` function documentation][FeaturesPagesFunction].

If you want to access the data of a particular page, use ComponentSample's `pages`
variable. Just take `content/_meta.md` in ComponentSample's sample contents for an
...
`{{ pages["_meta"].meta.logo }}`. Don't ever try to use ComponentSample's `pages` variable
as an replacement for ComponentSample's `pages()` function. Its usage looks very similar,
it will kinda work and you might even see it being used in old themes, but be
warned: It slows down ComponentSample. Always use ComponentSample's `pages()` function when iterating
ComponentSample's page list (e.g. `{% for page in pages() %}…{% endfor %}`).

#### Twig filters and functions

Additional to [Twig][]'s extensive list of filters, functions and tags, ComponentSample
...
* Arrays can be sorted by one of its keys using the `sort_by` filter
  ...
  `[ 'meta', 'nav' ]` part of the example, it instructs ComponentSample to sort by
...
ComponentSample makes it very easy for you to add new features to your website using
plugins. Just like ComponentSample, you can install plugins either using [Composer][]
(e.g. `composer require phrozenbyte/ComponentSample-file-prefixes`), or manually by
...
because it makes updating both ComponentSample and your plugins way easier. Anyway,
...
Plugins which were written to work with ComponentSample 1.0 and later can be enabled and
...
developed a plugin before and want to upgrade it to ComponentSample 2.0, refer to the
[upgrade section of the docs][PluginUpgrade].

## Config

Configuring ComponentSample really is stupidly simple: Just create a `config/config.yml`
to override the default ComponentSample settings (and add your own custom settings). Take
...
use a arbitrary number of config files. Simply create a `.yml` file in ComponentSample's
...
Please note that ComponentSample loads config files in a special way you should be aware
of. First of all it loads the main config file `config/config.yml`, and then
any other `*.yml` file in ComponentSample's `config` dir in alphabetical order. The file
order is crucial: Config values which have been set already, cannot be
overwritten by a succeeding file. For example, if you set `site_title: ComponentSample` in
`config/a.yml` and `site_title: My awesome site!` in `config/b.yml`, your site
title will be "ComponentSample".

Since YAML files are plain text files, users might read your ComponentSample config by
...
*always* make sure to configure your webserver to deny access to ComponentSample's
...
ComponentSample's `config` dir.

### URL Rewriting

ComponentSample's default URLs (e.g. %base_url%/?sub/page) already are very user-friendly.
Additionally, ComponentSample offers you a URL rewrite feature to make URLs even more
...
rewritten URLs work, but ComponentSample still shows no rewritten URLs, force URL
...
the `Options` directive from ComponentSample's `.htaccess` file (it's the last line).

#### Nginx

If you're using Nginx, you can use the following config to enable URL rewriting
(lines `5` to `8`) and denying access to ComponentSample's internal files (lines `1` to
`3`). You'll need to adjust the path (`/ComponentSample` on lines `1`, `2`, `5` and `7`)
...
config should provide the *bare minimum* you need for ComponentSample. Nginx is a very
extensive subject. If you have any trouble, please read through our
[Nginx config docs][NginxConfig].

```
location ~ ^/ComponentSample/((config|content|vendor|composer\.(json|lock|phar))(/|$)|(.+/)?\.(?!well-known(/|$))) {
    try_files /ComponentSample/index.php$is_args$args =404;
}

location /ComponentSample/ {
    index index.php;
    try_files $uri $uri/ /ComponentSample/index.php$is_args$args;
}
```

#### Lighttpd

ComponentSample runs smoothly on Lighttpd. You can use the following config to enable URL
rewriting (lines `6` to `9`) and denying access to ComponentSample's internal files (lines
`1` to `4`). Make sure to adjust the path (`/ComponentSample` on lines `2`, `3` and `7`)
to match your installation directory, and let ComponentSample know about available URL
rewriting by setting `rewrite_url: true` in your `config/config.yml`. The
config below should provide the *bare minimum* you need for ComponentSample.

```
url.rewrite-once = (
    "^/ComponentSample/(config|content|vendor|composer\.(json|lock|phar))(/|$)" => "/ComponentSample/index.php",
    "^/ComponentSample/(.+/)?\.(?!well-known(/|$))" => "/ComponentSample/index.php"
)

url.rewrite-if-not-file = (
    "^/ComponentSample(/|$)" => "/ComponentSample/index.php"
)
```

## Documentation

For more help have a look at the ComponentSample documentation at http://picocms.org/docs.

[ComponentSample]: http://picocms.org/
[PicoTheme]: https://github.com/picocms/ComponentSample-theme
[SampleContents]: https://github.com/picocms/ComponentSample/tree/master/content-sample
...
[WikiThemes]: https://github.com/picocms/ComponentSample/wiki/ComponentSample-Themes
[WikiPlugins]: https://github.com/picocms/ComponentSample/wiki/ComponentSample-Plugins
